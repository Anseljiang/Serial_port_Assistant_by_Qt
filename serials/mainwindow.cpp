...
MainWindow::MainWindow(QWidget *parent) :
    ...
    currentSettingsPath("/usr/lib/serialss"),
    RecSize(0),SendSize(0)
{
    ui->setupUi(this);
    serial = new serialsDispatcher();
    localHostName = QHostInfo::localHostName();
    Welcome = QString("Hello everyone! I am"+localHostName+"!");
    time = new QTimer;
...
void MainWindow::on_OpenCom_clicked()
{
    if(!portOpened)
    {
        ...
        serial->setRequestToSend(ui->RTScheckBox->isChecked());
         ...
         {        
             ...
             connect(serial->iface,SIGNAL(readyRead()),this,SLOT(readSerial()));//信号和槽函数连接，当串口缓冲区有数据时，进行读串口操作
             connect(time,SIGNAL(timeout()),this,SLOT(timeHandle()));
   ...
   else
    {
       ...
       serial->close();
       //猜测发送数据用的
       disconnect(serial->iface,SIGNAL(readyRead()),this,SLOT(readSerial()));
